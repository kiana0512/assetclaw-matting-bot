...
def test_workspace_roots_include_def_not_c() -> None:
...
def test_text_write_read_append_hash_and_batch_info() -> None:
    path = "E:\\assetclaw-matting-bot\\storage\\debug\\workspace_note.txt"
    ...
    batch = file_batch_info([path, "E:\\assetclaw-matting-bot\\storage\\debug\\missing.txt"])
    assert batch["count"] == 2


def test_copy_tree_and_dangerous_skills_require_confirmation() -> None:
    src = Path("E:/assetclaw-matting-bot/storage/debug/tree_src")
    dst = Path("E:/assetclaw-matting-bot/storage/debug/tree_dst")
...
def test_rename_sequence_preserves_extensions() -> None:
    root = Path("E:/assetclaw-matting-bot/storage/debug/rename_sequence")
...
def test_batch_file_operations_and_unzip() -> None:
    ...
    root = Path("E:/assetclaw-matting-bot/storage/debug/batch_ops")
    src_dir = root / "src"
    dst_dir = root / "dst"
