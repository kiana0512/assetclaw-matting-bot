...
def export_json(
    ...
    out = validate_path(
        output_path or "E:\\assetclaw-matting-bot\\storage\\feishu_table_exports\\animation_table.json",
        must_exist=False,
    )
    cfg = validate_path(
        config_path or "E:\\assetclaw-matting-bot\\feishu_frame_tool\\config.json",
        must_exist=True,
    )
...
def restore_plan(
    ...
    src = validate_path(
        json_path or "E:\\assetclaw-matting-bot\\storage\\feishu_table_exports\\animation_table.json",
        must_exist=True,
    )
    dst = validate_path(
        plan_output_path or "E:\\assetclaw-matting-bot\\storage\\feishu_table_exports\\animation_table_restore_plan.json",
        must_exist=False,
    )
