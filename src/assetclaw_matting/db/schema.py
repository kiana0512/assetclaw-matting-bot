...
CREATE INDEX IF NOT EXISTS idx_brain_msgs_conv     ON brain_messages(conversation_id);
CREATE INDEX IF NOT EXISTS idx_brain_msgs_provider ON brain_messages(provider);

-- ArkClaw-specific message log (legacy compat)
CREATE TABLE IF NOT EXISTS arkclaw_messages (
    id              INTEGER PRIMARY KEY AUTOINCREMENT,
    conversation_id TEXT,
    user_id         TEXT,
    message_text    TEXT,
...
-- Legacy agent_messages (kept for backward compatibility)
