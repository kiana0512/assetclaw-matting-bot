...
| `animation_flow.preview` | animation_flow | readonly | Yes | No | No | 预览完整 6 步动画自动化流程 |
| `animation_flow.start` | animation_flow | write_safe | Yes | No | Yes | 执行飞书下载 -> 抽帧 -> 抠图 -> unity_ready -> Unity 导入 -> P4 Shelve-only |
| `animation_flow.status` | animation_flow | readonly | Yes | No | No | 查看完整 6 步流程和子任务进度 |
| `animation_flow.list` | animation_flow | readonly | Yes | No | No | 列出完整 6 步流程任务 |
| `animation_flow.cancel` | animation_flow | write_safe | Yes | No | No | 终止完整 6 步流程和当前子任务 |
...
| `unity_tools.atlas_status` | unity | readonly | Yes | No | No | 查看 `Assets/TATest/AtlasSizeReport.json` 图集报告状态 |
| `unity_tools.atlas_report` | unity | write_safe | Yes | No | Yes | 调用 Unity 图集统计工具生成报告 |
| `unity_tools.rename_preview` | unity | readonly | Yes | No | No | 预览动画贴图批量重命名流水线 |
| `unity_tools.rename_run` | unity | write_safe | Yes | No | Yes | 执行动画贴图批量重命名流水线 |
