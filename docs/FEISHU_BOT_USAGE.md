...
直传图片/视频的 Cherry 后处理算法以 `E:\imageclip-pipeline\imageclip\cherry-postprocess.html` 为准。机器人优先用无头浏览器调用这份 HTML；如果浏览器流程失败，再使用 Python 兜底逻辑。兜底逻辑也必须按这份 HTML 的正方形/长方形默认预设对齐。
