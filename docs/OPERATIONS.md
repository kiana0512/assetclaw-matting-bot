...
“完整重跑”适用于已结束或失败的图片、序列帧和视频直发任务。它与“完整重发”的区别是：

- 完整重发只重新打包和发送已有产物，不重新占用抠图和后处理资源；
- 完整重跑从原始素材重新执行抽帧（视频）、抠图、后处理、打包和发送。

完整重跑必须在站内弹窗二次确认。确认后系统复用原任务 ID、原列表位置、原飞书收件人
和处理参数，原地把状态更新为 `QUEUED/RUNNING`，不会创建第二行任务。上一次的
`status/stage/error/children/delivery` 会写入有界 `rerun_history`；旧产物在重算前安全归档
或清空，但原始素材不删除。重复点击同一活动任务会被幂等检查拦截。

任务中心还会将同一飞书用户重复上传的完全相同素材（顺序、文件名和大小一致）折叠为
一个业务任务：显示最新执行状态，旧失败父任务和旧 ComfyUI/Cherry 子批次仅保留在审计
信息中，不再作为独立失败行展示。

运行中任务的“取消”使用站内确认弹窗，显示任务、用户和当前阶段。取消只停止当前执行，
不会删除原始素材、处理记录或已生成文件，取消后仍可在同一行执行完整重跑。
