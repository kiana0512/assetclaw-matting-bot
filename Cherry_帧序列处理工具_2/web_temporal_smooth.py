...
def temporal_smooth(imgs, 窗口大小=5, 平滑强度=1.0, 同步修正RGB=False, 最小Alpha保护=0.05, 环形宽度=25):
    imgs = imgs.float().clamp(0, 1)
    B, H, W, C = imgs.shape
    if C < 4: return imgs
    r = (窗口大小 - 1) // 2
    ...
    for t in range(B):
        ...
        ao[t] = torch.where(alpha[t] > 0, ao[t], torch.zeros_like(ao[t]))
        if 同步修正RGB:
...
<div class="module active" id="mod-denoise">
  ...
  <div class="module-body">
    <div class="param-row">
      <label>阈值</label>
      <input type="range" id="p-dn-thresh" min="0.0" max="0.5" step="0.01" value="0.06">
...
<script>
// ── 模块开关 ──────────────────────────────────────────────────────────────
const moduleState = { denoise: true, smooth: true, resize: true, sharp: true };
...
document.getElementById('btn-process').addEventListener('click', async ()=>{
  ...
  fd.append('dn_thresh',    document.getElementById('p-dn-thresh').value);
  fd.append('dn_radius',    document.getElementById('p-dn-radius').value);
  ...
  fd.append('ring_width',   document.getElementById('p-ring').value);
...
def process():
    try:
        use_denoise  = request.form.get("use_denoise", "1") == "1"
        dn_thresh    = float(request.form.get("dn_thresh", 0.06))
        dn_radius    = int(request.form.get("dn_radius", 0))
        ...
        ring_width   = int(request.form.get("ring_width", 25))
